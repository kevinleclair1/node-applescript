...
function runApplescript(strOrPath, args, callback) {
  ...
  interpreter.on('exit', function(code) {
    if (interpreter.stdout.body.charAt(0) === '{'){
      //in case applescript attempts to return a JSON object, currently breaks array functionality
      var result = interpreter.stdout.body;
    }
    else {
